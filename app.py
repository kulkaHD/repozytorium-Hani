...
class db:
    ...
    def merge(self):
        ...
        self.merged = df
        df['tran_date_nazwa_dnia'] = df['tran_date'].dt.weekday.map({0: 'Poniedziałek',
                                                              1: 'Wtorek',
                                                              2: 'Środa',
                                                              3: 'Czwartek',
                                                              4: 'Piątek',
...
app.layout = html.Div([html.Div([dcc.Tabs(id='tabs',value='tab-1',children=[
                            ...
                            dcc.Tab(label='Klienci',value='tab-4'),
                            ]),
...
def tab3_pie_store_type(chosen_cat):
    ...
    grouped = df.merged[df.merged['total_amt']>0].groupby(['Store_type','tran_date_nazwa_dnia'])['total_amt'].sum()
    grouped_store = grouped[chosen_cat]
    fig = go.Figure(data=[go.Pie(labels=grouped_store.index,values=grouped_store.values)],layout=go.Layout(title='według kanałów sprzedaży'))
    return fig

@app.callback(Output('pie-dzien','figure'), [Input('dzien_dropdown','value')])
def tab3_pie_dzien(chosen_cat):

    grouped = df.merged[df.merged['total_amt']>0].groupby(['tran_date_nazwa_dnia','Store_type'])['total_amt'].sum()
    grouped_dzien = grouped[chosen_cat]
    fig = go.Figure(data=[go.Pie(labels=grouped_dzien.index,values=grouped_dzien.values)],layout=go.Layout(title='według dni tygodnia'))
    return fig
...
def tab4_pie_gender(chosen_cat):
    ...
    fig = go.Figure(data=[go.Pie(labels=grouped_gender.index,values=grouped_gender.values)],layout=go.Layout(title='według płci'))
    return fig
...
def tab4_pie_country(chosen_cat):
    ...
    fig = go.Figure(data=[go.Pie(labels=grouped_country.index,values=grouped_country.values)],layout=go.Layout(title='według kraju zakupu'))
    return fig
